...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{fig4_sim_platform}
  \caption{仿真平台架构图}
...
外部交互与监控模块的功能定位是把仿真平台与控制算法暴露给用户/上层脚本，使工况切换、目标修改与运行监控不需要改动控制器代码。模块包括 \textbf{UDP 指令服务器}、\textbf{状态打印/日志}与\textbf{键盘控制}三个子组件（其与控制器目标缓存、主循环的关系已在\cref{fig:simplatform}「外部交互」侧栏中给出）。UDP 指令服务器在固定端口监听速度、yaw、腿长等目标值的更新，将其热更新到控制器的目标缓存；状态打印/日志组件将状态估计器的关键变量按设定周期输出，便于在不同工况下进行响应曲线绘制；键盘控制组件则提供运行时的人工干预通道。这一模块虽不参与控制计算本身，但其存在保证了「同一份控制器代码 + 同一仿真二进制 + 多种实验工况」的实验工作流。
...
\end{equation}
其中惯量矩阵 $\mathbf{M}(\cdot)$ 与右端项 $\mathbf{h}(\cdot)$ 由轮平动、车身平动、车身转动以及虚拟摆杆转动四类拉格朗日方程整理而得。为使后续 LQR/MPC 设计可追溯，下面给出其展开形式（取 $L = L_M = L_0/2$，并定义有效质量系数 $\xi := M + m_p/2$、轮等效转动惯量 $I_w = m_w R^2$、虚拟摆杆等效转动惯量 $I_p = m_p\,(L_0^2 + 0.05^2)/12$）。惯量矩阵的三行三列依次对应「轮平动 / 摆杆转动 / 车身 pitch」三个方程：
\begin{equation}
\label{eq:M-mat}
\mathbf{M}(\mathbf{x}, L_0) =
\begin{bmatrix}
R\,\xi\,L_0\,\cos\theta            & R(M + m_p + 2 m_w)        & -R\,M\,l\,\cos\varphi \\[2pt]
I_p + L_0^2\,(M + m_p/4)            & -\tfrac{L_0}{2}(2M+m_p)\cos\theta & M\,l\,L_0\,\cos(\theta-\varphi) \\[2pt]
M\,L_0\,l\,\cos(\theta+\varphi)     & M\,l\,\cos\varphi          & I_M + M\,l^2
\end{bmatrix}
\end{equation}
矩阵的非对角元体现了三类耦合：第一行的 $\cos\theta$ 项耦合摆杆姿态与轮平动；第二行的 $\cos(\theta-\varphi)$ 项耦合摆杆与车身倾角；第三行的 $\cos(\theta+\varphi)$ 项体现了车身倾角对摆杆产生的反向耦合。右端项 $\mathbf{h}(\cdot)$ 由控制输入、重力项与离心/科氏项三部分组成，引入两组辅助记号：
\begin{align}
\label{eq:dyn-aux-P}
P_\mathrm{eq}   &= (M+m_p)\,g - \xi L_0\,\dot\theta^2\cos\theta - M l\,\dot\varphi^2\cos\varphi,\\
\label{eq:dyn-aux-PM}
P_\mathrm{eq,M} &= M\,g - M L_0\,\dot\theta^2\cos\theta - M l\,\dot\varphi^2\cos\varphi,\\
\label{eq:dyn-aux-N}
N_\mathrm{eq}   &= -\xi L_0\,\dot\theta^2\sin\theta + M l\,\dot\varphi^2\sin\varphi,\quad
N_\mathrm{eq,M} = -M L_0\,\dot\theta^2\sin\theta + M l\,\dot\varphi^2\sin\varphi.
\end{align}
则
\begin{equation}
\label{eq:h-vec}
\mathbf{h}(\mathbf{x}, T, T_p, L_0) =
\begin{bmatrix}
T + R\bigl(\xi L_0\,\dot\theta^2 \sin\theta - M l\,\dot\varphi^2 \sin\varphi\bigr) \\[2pt]
\tfrac{L_0}{2}\,(P_\mathrm{eq} + P_\mathrm{eq,M})\,\sin\theta - \tfrac{L_0}{2}\,(N_\mathrm{eq} + N_\mathrm{eq,M})\,\cos\theta - T + T_p \\[2pt]
T_p + N_\mathrm{eq,M}\,l\,\cos\varphi + P_\mathrm{eq,M}\,l\,\sin\varphi
\end{bmatrix}
\end{equation}
式~\eqref{eq:M-mat}--\eqref{eq:h-vec} 在数值上与本研究使用的 \texttt{calc\_lqr\_k.py} 中 \texttt{dynamics(\ldots)} 函数（变量名 \texttt{c\_*\_dd*} 对应 $\mathbf{M}$ 各分量、\texttt{rhs\_*} 对应 $\mathbf{h}$ 各分量、\texttt{P\_base/PM\_base/N\_nl/NM\_nl} 对应辅助量~\eqref{eq:dyn-aux-P}--\eqref{eq:dyn-aux-N}）完全一致；式~\eqref{eq:dyn-mass} 等价于一个非线性状态空间方程：
\begin{equation}
\label{eq:dyn-ss}
...
仿真主循环（\texttt{Simulation.py}）按整数倍关系协同推进四类子任务：物理仿真步进 \texttt{mj\_step}（每次循环执行）、传感器读取（每 1\,ms 一次）、控制计算（每 4\,ms 一次）、监控/键盘（每 20\,ms 一次）。控制周期之所以取 4\,ms（250\,Hz）而非更高频率，是为了与实物常用的电机驱动周期一致，便于后续迁移；监控周期取 20\,ms 则避免标准输出对主循环造成阻塞。仿真平台运行时的可视化界面与控制台输出见\cref{fig:mjsim}。
...
PID 控制器（\texttt{PIDController.py}）采用四级联结构（见\cref{fig:pidcascade}：位置外环 $\to$ pitch 内环 $\to$ 轮力矩；yaw PID 差分；VMC 逆 $\to$ 关节内环）。设期望腿长 $L_0^d$ 与期望腿极角 $\varphi_0^d = \pi/2$（站立位），由 VMC 逆运动学得到关节目标 $\varphi_1^d, \varphi_4^d$，关节内环按经典 PID 计算关节力矩：
\begin{equation}
\label{eq:pid-joint}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{fig7_pid_cascade}
...
\end{equation}
权重矩阵取 $Q = \mathrm{diag}(50, 1, 800, 100, 7000, 1)$、$R = \mathrm{diag}(20, 1)$，体现「车身倾角与位移误差优先收敛、轮力矩适度抑制」的设计偏好。CARE 求解使用 \texttt{scipy.linalg.solve\_continuous\_are}。离线打表得到的 $K$ 矩阵各分量随 $L_0$ 的变化曲线见\cref{fig:lqrk}，可以看到 pitch 通道增益 $K_{1,5}$ 在 $L_0 \in [0.10, 0.40]\,\mathrm{m}$ 范围内显著变化，固定增益反馈在大腿长偏移下确有失稳风险。

LQR 离线打表与运行时插值的整体流程参见\cref{alg:lqr-tab}。为保证左右腿独立控制并避免互扰，控制器对左、右腿分别取一份状态向量并各运行一路 LQR 反馈，得到两路 $T_p$（分别作用于左右髋）；轮力矩 $T$ 取两路平均后再叠加 yaw PID 差分。

\begin{algorithm}[H]
\caption{LQR $K(L_0)$ 离线打表 + 运行时线性插值}\label{alg:lqr-tab}
\begin{algorithmic}[1]
\REQUIRE 物理参数 $\theta_\mathrm{p}$；权重矩阵 $Q, R$；样点数 $N_K$；区间 $[L_0^\mathrm{lo}, L_0^\mathrm{hi}]$
\ENSURE 控制输入 $\mathbf{u} = (T, T_p)^T$
\STATE \textbf{阶段 A：离线打表}
\STATE $\{L_0^{(i)}\}_{i=1}^{N_K} \leftarrow$ 在 $[L_0^\mathrm{lo}, L_0^\mathrm{hi}]$ 等距采样
\FOR{$i = 1, \dots, N_K$}
  \STATE $(A_i, B_i) \leftarrow$ 对 $f(\cdot;L_0^{(i)})$ 在 $\mathbf{x}=\mathbf{0}, \mathbf{u}=\mathbf{0}$ 做有限差分线性化
  \STATE $P_i \leftarrow \mathtt{solve\_continuous\_are}(A_i, B_i, Q, R)$
  \STATE $K_i \leftarrow R^{-1} B_i^T P_i$
\ENDFOR
\STATE 写入 \texttt{lqr\_config.json}：$\{(L_0^{(i)}, K_i)\}$
\STATE \textbf{阶段 B：运行时（每个控制周期 $T_c$ 执行一次）}
\STATE 读取当前实测 $L_0$；二分查找 $i$ 使 $L_0^{(i)} \le L_0 < L_0^{(i+1)}$
\STATE $t \leftarrow (L_0 - L_0^{(i)}) / (L_0^{(i+1)} - L_0^{(i)})$
\STATE $K(L_0) \leftarrow (1-t)\,K_i + t\,K_{i+1}$
\STATE 由状态估计器读取 $\mathbf{x}$；$\mathbf{u} \leftarrow -K(L_0)\,\mathbf{x}$
\RETURN $\mathbf{u}$
\end{algorithmic}
\end{algorithm}
...
\textbf{求解器}采用\textbf{自实现的投影梯度下降（PGD）}，整体流程见\cref{fig:mpcflow}与\cref{alg:mpc-pgd}：以前一周期解整体左移一位作为本周期 warm start，PGD 步长取 $\alpha = 1/\lambda_{\max}(H)$，最多迭代 30 次：
\begin{equation}
\label{eq:pgd}
...
与 LQR 类似，预测模型 $(A_d, B_d, P)$ 也按 $L_0$ 离线打表（\texttt{mpc\_config.json}），运行时三者均按式~\eqref{eq:lqr-interp} 线性插值。左右腿仍各跑一路 MPC，各自维护 warm start。\cref{alg:mpc-pgd} 给出了 MPC 在每个控制周期内的完整求解流程，包括模型插值、矩阵装配、闭式可行性检查、warm-start 选择与 PGD 迭代。

\begin{algorithm}[H]
\caption{MPC condensed-QP + PGD 单步求解}\label{alg:mpc-pgd}
\begin{algorithmic}[1]
\REQUIRE 当前状态 $\mathbf{x}_0$；当前腿长 $L_0$；上周期解 $\mathbf{U}_\mathrm{prev}$；最大迭代次数 $K_\mathrm{max} = 30$
\ENSURE 当前周期控制输入 $\mathbf{u}_0 \in \mathbb{R}^{n_u}$
\STATE $(A_d, B_d, P) \leftarrow$ 按 $L_0$ 在 \texttt{mpc\_config.json} 中线性插值
\STATE 构造预测块矩阵 $S_x, S_u$ 与块对角权重 $\bar Q = \mathrm{blkdiag}(Q,\dots,Q,P)$，$\bar R = \mathrm{blkdiag}(R,\dots,R)$
\STATE $H \leftarrow 2(S_u^T \bar Q S_u + \bar R)$；$\quad g \leftarrow 2 S_u^T \bar Q S_x\, \mathbf{x}_0$
\STATE $\mathbf{U}^* \leftarrow -H^{-1} g$ \COMMENT{无约束闭式解}
\IF{$\mathbf{u}_\mathrm{min} \le \mathbf{U}^* \le \mathbf{u}_\mathrm{max}$ 逐元素成立}
  \STATE $\mathbf{U} \leftarrow \mathbf{U}^*$ \COMMENT{可行，跳过 PGD}
\ELSE
  \STATE $\mathbf{U}_\mathrm{shift} \leftarrow$ 将 $\mathbf{U}_\mathrm{prev}$ 整体左移一位，末位补零 \COMMENT{warm start}
  \STATE $\mathbf{U}^{(0)} \leftarrow \arg\min_{\mathbf{U} \in \{\mathbf{U}_\mathrm{shift}, \mathbf{U}^*\}} J(\mathbf{U})$
  \STATE $\alpha \leftarrow 1 / \lambda_\mathrm{max}(H)$
  \FOR{$k = 0, \dots, K_\mathrm{max}-1$}
    \STATE $\mathbf{U}^{(k+1)} \leftarrow \Proj_{[\mathbf{u}_\mathrm{min}, \mathbf{u}_\mathrm{max}]}\bigl(\mathbf{U}^{(k)} - \alpha\,(H \mathbf{U}^{(k)} + g)\bigr)$
  \ENDFOR
  \STATE $\mathbf{U} \leftarrow \mathbf{U}^{(K_\mathrm{max})}$
\ENDIF
\STATE $\mathbf{U}_\mathrm{prev} \leftarrow \mathbf{U}$ \COMMENT{保存供下周期 warm start}
\RETURN $\mathbf{u}_0 = \mathbf{U}[0:n_u]$
\end{algorithmic}
\end{algorithm}在未触发输入约束的工况下，MPC 第一步控制律 $\mathbf{u}_0$ 可视为关于 $\mathbf{x}_0$ 的线性反馈，其等效反馈增益 $K_\mathrm{MPC}$ 与 LQR 增益 $K_\mathrm{LQR}$ 的对比见\cref{fig:mpcvslqr}：两者在 $L_0$ 全程几乎重合，从数值上验证了「无约束 MPC ≡ LQR」这一理论结论，也说明 MPC 相比 LQR 的优势完全来自显式输入约束的存在。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{fig9_mpc_pgd_flow}
...
\begin{itemize}
\item \textbf{工况 1（平衡保持）}（响应曲线见\cref{fig:case1}）：所有目标量置零、$L_0^d = 0.20\,\mathrm{m}$，仿真时长 10\,s，重点考察各控制器在零工作点附近的稳态精度与抖动；
\item \textbf{工况 2（位置阶跃）}（响应曲线见\cref{fig:case2}）：$x^d$ 在 $t = 1\,\mathrm{s}$ 由 0 阶跃至 0.5\,m，重点考察位移响应的上升时间、超调与稳态误差；
\item \textbf{工况 3（速度跟踪）}（响应曲线见\cref{fig:case3}）：$\dot x^d$ 在 $t = 1\,\mathrm{s}$ 由 0 阶跃至 0.3\,m/s，进入「速度模式」（位置误差冻结），重点考察速度跟踪精度；
\item \textbf{工况 4（yaw 阶跃）}（响应曲线见\cref{fig:case4}）：$\mathrm{yaw}^d$ 由 0 阶跃至 $\pi/4\,\mathrm{rad}$，重点考察 yaw 跟踪与左右轮差分的协调；
\item \textbf{工况 5（腿长动态变化）}（响应曲线见\cref{fig:case5}）：$L_0^d$ 在 $[0.15, 0.35]\,\mathrm{m}$ 之间以 0.1\,Hz 正弦变化，重点考察 LQR 增益插值与 MPC 模型插值在时变模型下的稳定性；
\item \textbf{工况 6（瞬态扰动恢复）}（响应曲线见\cref{fig:case6}）：在车身上施加幅值 30\,N、持续 0.1\,s 的水平推力，重点考察轮力矩饱和场景下的扰动恢复能力。
\end{itemize}
...
\textbf{工况 1（平衡保持）}（响应曲线见\cref{fig:case1}） 三种控制器均能在 $L_0 = 0.20\,\mathrm{m}$ 标定点附近稳态平衡。PID 在 pitch 通道上呈现约 $\pm 0.5^\circ$ 的稳态抖动（来自位置外环的等效低增益）；LQR 与 MPC 由于直接利用六维状态信息，pitch 抖动小于 $\pm 0.1^\circ$，位移漂移亦更小。
...
\textbf{工况 2（位置阶跃）}（响应曲线见\cref{fig:case2}） 在 0.5\,m 位置阶跃下，PID 上升时间最短但伴随 pitch 反向超调（车身先后倾再前倾），LQR 通过状态反馈使 pitch 与位移协调收敛，MPC 在保持类似响应的同时使轮力矩更平滑。
...
\textbf{工况 3（速度跟踪）}（响应曲线见\cref{fig:case3}） 三种控制器均能跟踪 0.3\,m/s 的速度阶跃；PID 的稳态速度误差较大（约 0.04\,m/s），LQR 与 MPC 的稳态误差均在 0.01\,m/s 以内。
...
\textbf{工况 4（yaw 阶跃）}（响应曲线见\cref{fig:case4}） yaw 控制由共用的 yaw PID 完成，三种控制器在 yaw 通道的响应基本一致，差异主要体现在 yaw 转动期间车身平衡的协调上：PID 在 yaw 转动峰值期间 pitch 误差略增，LQR 与 MPC 几乎不受影响。
...
\textbf{工况 5（腿长动态变化）}（响应曲线见\cref{fig:case5}） 当 $L_0^d$ 在 $[0.15, 0.35]\,\mathrm{m}$ 之间正弦变化时，PID 由于反馈增益固定，pitch 通道在 $L_0$ 偏离 0.20\,m 时呈现持续小幅振荡；LQR 借助式~\eqref{eq:lqr-interp} 的增益插值保持平稳；MPC 模型同步插值后表现与 LQR 相当，且轮力矩更平滑。这一工况是体现 LQR/MPC「时变线性化 + 增益调度」设计价值的核心实验。
...
\textbf{工况 6（瞬态扰动恢复）}（响应曲线见\cref{fig:case6}） 30\,N、0.1\,s 的水平推力会显著激发车身倾角偏离。PID 与 LQR 均会产生瞬时 $|T| \to 4\,\mathrm{N\cdot m}$ 的轮力矩饱和，但 LQR 由于权重 $R$ 较小，饱和持续时间更长；MPC 通过式~\eqref{eq:mpc-constraints} 的盒约束在饱和边界自适应分配 $T$ 与 $T_p$，pitch 误差峰值显著低于另外两者。
